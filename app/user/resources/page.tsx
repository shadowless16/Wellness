  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-white/80 to-warm-beige/5">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-light mb-3 bg-gradient-to-r from-[oklch(0.70_0.15_50)] to-[oklch(0.65_0.15_130)] bg-clip-text text-transparent">Learning Resources</h1>
          ...
          {resources.map((resource) => {
            ...
            return (
              <Link key={resource.id} href={`/user/resources/${resource.id}`}>
                <div className={`${resource.bgColor} rounded-[24px] p-6 cursor-pointer transition-all hover:shadow-lg hover:scale-105 duration-300 border border-[oklch(0.70_0.15_50)]/20 h-full flex flex-col`}>
                  ...
                  <div className="pt-4 border-t border-[oklch(0.70_0.15_50)]/20">
                    <p className="text-xs text-muted-foreground mb-1">{resource.author}</p>
                    <p className="text-xs font-medium text-[oklch(0.70_0.15_50)]">{resource.readTime}</p>
                  </div>
                </div>
              </Link>
            )
